...
function createJson (options, xmlObject) {
  ...
  fs.writeFileSync(`${fileName}.json`, JSON.stringify(xmlObject, null, '\t'));
}
...
function createYaml (options, xmlObject) {
  ...
  fs.writeFileSync(`${fileName}.yaml`, yaml.stringify(xmlObject, 4));
}
...
function createHtml (options, xmlObject, dependencyLicenseFiles) {
  ...
  xmlObject.dependencies.dependency.forEach(d => {
    ...
    if (dependencyFound) {
      ...
      d.localLicense = path.join(dir, finalName);
    } else {
      d.localLicense = 'N/A';
    }
  });
