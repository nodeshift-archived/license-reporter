...
function createHtml (options, xmlObject, dependencyLicenseFiles) {
  const findDependency = (name, type) => dependencyLicenseFiles.find(d => d.name === name && d.type === type);
  const notUsingReadmeAsLicenseFile = (name) => dependencyLicenseFiles.find(d => {
    return d.name === name && (d.file && path.basename(d.file) !== 'README.md');
  });

  xmlObject.dependencies.dependency.forEach(d => {
    const dependencyFound = findDependency(d.packageName, d.licenses.license[0].name);
    if (dependencyFound) {
      const finalName = `${dependencyFound.name}_${dependencyFound.type}.txt`;
      const link = path.join('./', finalName);
      if (notUsingReadmeAsLicenseFile(d.packageName)) {
